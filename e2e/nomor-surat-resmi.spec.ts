...
function getSuratNomorPayload(pengajuanId: number): {
    ...
    const output = runTinker([
        ...
        `$html = view(\\App\\Models\\SuratTerbit::resolveTemplateView($p->jenisSurat?->nama_surat), [`,
        `'pengajuan' => $p,`,
        `'pemohon' => $p->user,`,
        ...
        `'pdf_contains_kode' => str_contains($pdf, (string) config('desa.kode_klasifikasi', '470')),`,
        `]);`,
    ].join('')).trim();
