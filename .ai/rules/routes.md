...
Warga-only routes pengajuan-surat.unduh-surat and pengajuan-surat.cetak-surat under role:warga. Owner-only; status must be diproses|siap_diambil|selesai via PengajuanSurat::dapatUnduhSurat(); call pastikanFilePdf() then serve — never mint new QR/token. Unduh=download attachment; cetak=inline PDF response.
